...
class Predictor:
    ...
    def _limit_override(self, df: pd.DataFrame) -> int | None:
        ...
        if np.isclose(upper_probe, UPPER_LIMIT, atol=LIMIT_ATOL):
            return 0
        if np.isclose(lower_probe, LOWER_LIMIT, atol=LIMIT_ATOL):
            return 2
        return None
    ...
    def _apply_threshold(self, proba: np.ndarray, label: str) -> int:
        ...
        top = float(proba.max(axis=1)[0])
        return 1 if top < threshold else pred
